...
function encode(prefix, type, hash) {
  (0, _ensure2.default)(typeof prefix === 'string', 'Invalid prefix: ' + prefix + '.');
  (0, _ensure2.default)(typeof type === 'string', 'Invalid type: ' + type + '.');
  (0, _ensure2.default)(hash instanceof Array, 'Invalid hash: ' + hash + '.');
...
function decode(address) {
  (0, _ensure2.default)(typeof address === 'string', 'Invalid address: ' + address + '.');
  var pieces = address.split(':');
  (0, _ensure2.default)(pieces.length === 2, 'Missing prefix: ' + address + '.');
  var prefix = pieces[0];
  var encodedPayload = pieces[1];
  (0, _ensure2.default)(hasSingleCase(encodedPayload), 'Mixed case in address payload: ' + encodedPayload + '.');
  var payload = base32.decode(encodedPayload.toLowerCase());
  (0, _ensure2.default)(validChecksum(prefix, payload), 'Invalid checksum: ' + address + '.');
  ...
  (0, _ensure2.default)(getHashSize(versionByte) === hash.length * 8, 'Invalid hash size: ' + address + '.');
